...
- **tick-to-fill ≈ 77 µs**, and this is where the matching engine shows up honestly. Under the old loopback fill this was ~1.75 µs (just the strategy→execution hop). Now the order round-trips to a real order book: it waits for the exchange's match loop to drain it, crosses the spread against resting liquidity, and a fill comes back. Most of the 77 µs is *not* transport — it is the order sitting until the exchange's next match/publish round. That is the true cost of going to a venue rather than filling in-process, and it is the right thing to see. (This figure depends on order density: it is a cold-path number under the sparse, throttled flow above. Denser flow — e.g. `ORDER_UNITS=1` with a short cooldown — keeps the exchange's loop hot and the same window reads a few µs. The point is the *shape* — decision and order-hop are fixed; the fill tracks how busy the venue is.)
...
The strategy console (`make studio`, then open `:8080`) shows these three windows and the decision-vs-transport breakdown live while you tune the strategy's weights, threshold and risk cap and watch orders, fills and P&L stream in. It also renders the strategy as a read-only node graph, and lights up the path each order fires through — the buy branch on a buy, the sell branch on a sell — so you can watch the decision flow through the graph as the pipeline runs. A toggle switches between the **composite** blend (the default, driven by the sliders) and a rule-based **gate** strategy — boolean conditions combined with **AND / OR** gates — which is compiled to real AND/OR bytecode and executed via the VM (`examples/gates.json`; inspect it with `cargo run --bin disasm -- examples/gates.json`).
